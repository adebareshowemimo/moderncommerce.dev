...
Modern Commerce adds an optional **webhook IP whitelist** (enabled by default) and a **payment max-retries** setting. Configure both under the native plugin settings; see [Admin Settings Reference](/{{route}}/{{version}}/modern-commerce/admin-settings). IP allow-listing is an extra control, not a replacement for signature verification.
